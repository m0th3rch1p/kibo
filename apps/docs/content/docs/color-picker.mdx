...
const Example = () => {
  ...
  return (
    <div className="flex w-full h-screen justify-center items-start gap-4 bg-secondary p-4">
      <Popover>
        <PopoverTrigger>
          <div className="relative">
            <div className="w-6 h-6 rounded border border-foreground/10 bg-secondary absolute top-1/2 left-2 -translate-y-1/2" style={{ backgroundColor: color }} />
            <Input placeholder="Pick a color" value={color} onChange={(e) => setColor(e.target.value)} className="pl-10 bg-background" />
          </div>
        </PopoverTrigger>
        <PopoverContent>
          <ColorPicker className="w-full" value={color} onValueChange={handleValueChange}>
            <ColorPickerSelection />
            <div className="flex items-center gap-4">
              <ColorPickerEyeDropper />
              <div className="w-full grid gap-1">
