...
// Memoized date functions to avoid recreation on every call
const rangeFunctionMemo = new Map<
	Range,
	{
		getDaysIn: (date: Date) => number;
		getDifferenceIn: (dateLeft: Date, dateRight: Date) => number;
		getInnerDifferenceIn: (dateLeft: Date, dateRight: Date) => number;
		getStartOf: (date: Date) => Date;
		getEndOf: (date: Date) => Date;
		getAddRange: (date: Date, amount: number) => Date;
	}
>();
...
const getRangeFunctions = (range: Range) => {
	...
	const functions = {
		getDaysIn: range === "daily" ? (_date: Date) => 1 : getDaysInMonth,
		getDifferenceIn: range === "daily" ? differenceInDays : differenceInMonths,
		getInnerDifferenceIn:
			range === "daily" ? differenceInHours : differenceInDays,
		getStartOf: range === "daily" ? startOfDay : startOfMonth,
		getEndOf: range === "daily" ? endOfDay : endOfMonth,
		getAddRange: range === "daily" ? addDays : addMonths,
	};

	rangeFunctionMemo.set(range, functions);
	return functions;
};
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
	...
	const { getDaysIn, getAddRange } = getRangeFunctions(context.range);
	const month = getAddRange(timelineStartDate, offset);
	const daysInMonth = getDaysIn(month);
...
const getOffset = (
	...
	const { getDifferenceIn, getStartOf } = getRangeFunctions(context.range);
	const fullColumns = getDifferenceIn(getStartOf(date), timelineStartDate);
...
const getWidth = (
	...
	const { getDifferenceIn, getInnerDifferenceIn, getStartOf } =
		getRangeFunctions(context.range);

	if (context.range === "daily") {
		const delta = getDifferenceIn(endAt, startAt);
	...
	if (isSameDay(getStartOf(startAt), getStartOf(endAt))) {
		return getInnerDifferenceIn(endAt, startAt) * pixelsPerDayInStartMonth;
	}

	const startRangeOffset = daysInStartMonth - getDate(startAt);
	const endRangeOffset = getDate(endAt);
	const fullRangeOffset = getDifferenceIn(
		getStartOf(endAt),
		getStartOf(startAt),
	);
...
const calculateInnerOffset = (
	...
	const { getStartOf, getEndOf, getInnerDifferenceIn } =
		getRangeFunctions(range);
	const startOfRange = getStartOf(date);
	const endOfRange = getEndOf(date);
	const totalRangeDays = getInnerDifferenceIn(endOfRange, startOfRange);
