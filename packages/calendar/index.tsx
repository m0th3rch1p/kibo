...
// Memoize expensive date formatting operations
const daysForLocaleCache = new Map<string, string[]>();

export const daysForLocale = (
	locale: Intl.LocalesArgument,
	startDay: number,
) => {
	const cacheKey = `${JSON.stringify(locale)}-${startDay}`;
	const cached = daysForLocaleCache.get(cacheKey);
	if (cached) {
		return cached;
	}
	...
	daysForLocaleCache.set(cacheKey, weekdays);
	return weekdays;
};
